...
Ein Next.js-Frontend für eine interne **Produkt-Wissensdatenbank / Ersatzteil-Wiki**.
Es zeigt einen durchsuchbaren, kategorisierten Produktkatalog an, der von
einem Strapi-5-Backend mit Postgres versorgt wird — inklusive persönlicher
Favoriten, eigener Listen, verknüpfter Ersatzteile und einem
Admin-Bereich für Inhalts- und Benutzerverwaltung.
...
Strapi übernimmt die CMS-/Datenbankschicht; dieses Repository ist die
kundenseitige Next.js-Anwendung darüber. Ihre Aufgabe:

- Den Produktkatalog darstellen (gruppiert nach Kategorie/"Rubrik",
  durchsuchbar nach Name, Artikelnummer, EAN oder ID).
- Eine Detailseite pro Produkt zeigen: Beschreibung, Bilder, Dokumente,
  Tags sowie beidseitig verknüpfte Ersatz-/Kompatibilitätsteile.
- Angemeldeten Nutzern erlauben, Artikel zu favorisieren und eigene
  "Listen" zu erstellen (z. B. für ein Projekt/eine Bestellung) — ganz ohne
  eigene Backend-Speicherung.
...
- Administratoren einen Admin-Bereich geben: Benutzerverwaltung,
  ...
  Ordner-Scan sowie eine Build-/Neustart-Steuerung für den Produktivserver —
  alles direkt in der App, ohne für den Tagesbetrieb das Strapi-Admin-Panel
  öffnen zu müssen.
- Zwei UI-Sprachen unterstützen (Englisch/Deutsch) sowie grundlegende
  DSGVO-Seiten (Cookie-Banner, Impressum, Datenschutz).
...
```
 Browser
   │  HTTPS
   ▼
 Next.js-App (dieses Repository)   ← läuft als PM2-Cluster über server.js, Port 3000
   │  app/api/*-Routen halten die EINZIGEN Strapi-Zugangsdaten
   │  (STRAPI_TOKEN); der Browser spricht nie direkt mit Strapi
   ▼
 Strapi-5-Backend (eigenes Repository/Host — siehe kukkineko/strapi-monorepo)
...
Backend-Host: hochgeladene Medien (`public/uploads`, einfache lokale
Festplatte) und ein kleiner In-Process-Response-Cache (Plugin
...
| Backend | Strapi 5.42.1 (eigenes Repository, `~/strapi-monorepo` in WSL/VPS) |
| Datenbank | PostgreSQL, vollständig im Besitz von Strapi — siehe [docs/database.md](docs/database.md) |
| Auth | Eigenes JWT-basiertes Auth-System gegen eine einzelne Strapi-Collection `appuser` (nicht Strapis eigene Admin-User, nicht die User dieses Plugins users-permissions) |
...
- Die Startseite gruppiert Einträge nach Kategorie ("Rubrik") mit
  Artikelanzahl je Bereich; eine globale Suche durchsucht
  Name/Artikelnummer/EAN/ID
  ([app/page.tsx](app/page.tsx), [app/components/home-page-client.tsx](app/components/home-page-client.tsx)).
- `/products/all` — flache, sortierbare Liste aller Einträge.
- `/products/[id]` — Produkt-Detailseite: Beschreibung, Bilder, Dokumente,
  Tags sowie beidseitige Querverweise auf kompatible/Ersatzteile.
...
- **Tabs & Split-Pane-Ansicht**: Über die Kopfleiste lassen sich Produkte in
  ...
  Das ersetzt eine frühere eigenständige `/compare`-Seite.
...
- **Listen** (`/listen` — benannt nach dem deutschen Wort *Listen*) — eigene
  Projekte anlegen und Artikel mit Menge/Positionsnummer hinzufügen,
  vollständig clientseitig verwaltet und im Benutzerdatensatz gespeichert
  ([app/lib/lists.ts](app/lib/lists.ts), [app/lib/list-types.ts](app/lib/list-types.ts)).
- **"Zur Liste hinzufügen"-Button**, überall verfügbar, wo ein Produkt
  angezeigt wird
...
| DB Backup | Vollständiges Datenbank-Backup exportieren/importieren |
...
| Server | Aus dem Browser heraus einen Build und Neustart des produktiven PM2-Prozesses auslösen |
...
> Drittanbieters mehr benötigt — siehe bei Bedarf die Git-Historie rund um
> die `import-data`-Routen.
...
- **Seitenaufruf-Statistik**: ein schlanker, einmal pro Nutzer und 24 Stunden
  ...
  Frontend-Server (nicht in Strapi — siehe den Hinweis "why a file" in
  [app/lib/page-views.ts](app/lib/page-views.ts)).
- **Rate Limiting**: ein minimaler, In-Memory Sliding-Window-Begrenzer für
  Login/Registrierung ([app/lib/rate-limit.ts](app/lib/rate-limit.ts)).
...
- `POST /api/auth/login` und `POST /api/auth/register` (in diesem
  Repository) rufen Strapis eigene Endpunkte `/api/app-auth/login` /
  `/register` auf, die das Passwort per bcrypt hashen/prüfen und ein JWT
  ausstellen, signiert mit einem serverseitigen `JWT_SECRET` bei Strapi.
...
- `app/components/auth-gate.tsx` steuert clientseitig gerenderte Inhalte
  abhängig vom Anmelde-/Rollenstatus.
...
```
app/
  page.tsx, layout.tsx           Startseite & Root-Layout
  products/                      Katalog: Übersicht, Detail, Bearbeiten, Neu
  listen/                        Persönliche "Listen"-Funktion
  ...
  api/                           Rein serverseitige Route-Handler (einziger
    ...
    auth/                        Login/Registrierung/Session/Admin-Nutzerverwaltung
  ...
  lib/                           Framework-unabhängige Server-/geteilte Logik
    ...
    rate-limit.ts                In-Memory-Rate-Limiter fürs Login
...
Die lokale Entwicklung an diesem Projekt findet **innerhalb von WSL**
(Ubuntu) statt, nicht direkt unter Windows — geklont/bearbeitet wird unter
`~/strapi-monorepo/frontend` in WSL, nicht der Windows-seitige Checkout,
sonst erkennt der Datei-Watcher von `next dev` Änderungen nicht (das gilt
besonders für CSS, das über einen UNC-Pfad von Windows aus bearbeitet
wurde). Das passende Strapi-Backend läuft aus `~/strapi-monorepo` (der in
diesem Dokument mehrfach genannte Pfad `/strapi`) in derselben
WSL-Umgebung, beide verwaltet unter PM2 als normaler (nicht-root) Nutzer.

```bash
# innerhalb von WSL, im Frontend-Package
npm install
...
| `STRAPI_TOKEN` | ja | Strapi-API-Token — voller Lese-/Schreibzugriff auf `/api/entries*`. **Nur serverseitig, niemals mit `NEXT_PUBLIC_` versehen.** |
| `COOKIE_SECURE` | nein | Erzwingt `Secure` beim Session-Cookie außerhalb der automatischen `NODE_ENV=production`-Erkennung |
| `VIEW_DATA_DIR` | nein | Überschreibt den Speicherort von `page-views.json` (Standard: `<cwd>/data`) |
| `PORT` | nein | Port für `server.js` (Standard: 3000) |

> Stand jetzt werden `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` **nicht mehr
> benötigt** — die KI-gestützte Import-Funktion, die sie nutzte, wurde
> entfernt.
...
Einzel-Worker-Modus ("Produktiv-Build, aber nicht der echte Prod-Cluster")
gibt es `pm2.dev.config.js` (`npm run start:dev`).

Wichtige Einschränkungen pro Worker-Prozess, bevor über einen einzelnen
Host/eine Prozessgruppe hinaus skaliert wird: Der Login-/Registrierungs-
Rate-Limiter und der Seitenaufruf-Zähler sind **pro Prozess, nicht geteilt**
— siehe die Kommentare in [app/lib/rate-limit.ts](app/lib/rate-limit.ts) und
[app/lib/page-views.ts](app/lib/page-views.ts). Für einen einzelnen
...
- Secrets liegen aktuell im Klartext in `.env.local`; sie sollten vor dem
  Deployment in den AWS Secrets Manager / SSM Parameter Store wandern.
  Insbesondere `STRAPI_TOKEN` ist zur Rotation vorgemerkt (ein früherer Wert
  wurde an Browser ausgeliefert).
- Hochgeladene Medien (`public/uploads` auf dem Strapi-Host, ~1 GB und
  wachsend) liegen aktuell auf einfacher lokaler Festplatte ohne
  Redundanz — hierfür wird entweder persistenter Block-/Netzwerkspeicher
  oder ein Wechsel auf einen S3-Upload-Provider benötigt, bevor der Betrieb
  auf vergänglichem Compute läuft.
