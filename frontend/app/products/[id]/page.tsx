...
import { getArtNrNeighbors, getEntryById, getMediaById, listEntries, normalizeRubrikValue, parseRubrikNumber, parseRubrikNumbers, parseLinkEntries, searchEntries, serializeLinkEntries, uploadMedia } from "@/app/lib/entries";
...
function displayRubrik(value: unknown): string {
  ...
  const nums = parseRubrikNumbers(value);
  if (nums.length > 0) return nums.map((n) => `R${String(n).padStart(2, "0")}`).join(", ");
...
export default function ProductPage() {
  ...
  async function handleToggleRubrik(value: string) {
    ...
    const currentNums = parseRubrikNumbers(entry.rubrik);
    const toggleNum   = parseRubrikNumber(value);
    let nextRubriks: string[];
    if (!value) {
      nextRubriks = [];
    } else if (toggleNum !== null) {
      const code = `R${String(toggleNum).padStart(2, "0")}`;
      nextRubriks = currentNums.includes(toggleNum)
        ? currentNums.filter((n) => n !== toggleNum).map((n) => `R${String(n).padStart(2, "0")}`)
        : [...currentNums.map((n) => `R${String(n).padStart(2, "0")}`), code];
    } else {
      nextRubriks = [value];
    }
                        ...
                        <div className="wiki-rubrik-dropdown">
                          {RUBRIK_OPTIONS.map((opt) => {
                            ...
                            const isActive    = opt.value === ""
                              ? currentNums.length === 0 && !normalizeRubrikValue(entry.rubrik)
                              : optNum !== null
                                ? currentNums.includes(optNum)
